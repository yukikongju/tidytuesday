...
### Finding out all the contestants with ___ personality type

```{r, echo=FALSE}
personality_sort <- function(type){
  players <- castaways %>% 
  filter(personality_type == type) %>% 
  distinct(full_name, .keep_all = TRUE) %>%
  select(-c("city", "state", "episode", "day", "order", "jury_status", "original_tribe", "swapped_tribe", "swapped_tribe_2", "merged_tribe"))
  return(players)
}

# remove NA from vector
personality_types <- unique(castaways$personality_type) 
personality_types <- personality_types[!is.na(personality_types)]

# storing players by personality types
for(type in personality_types){
  assign(type, personality_sort(type)) # ISTP <- personality_sort('ISTP')
}
```
